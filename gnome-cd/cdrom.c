...
static gboolean
cdrom_next (GnomeCDRom * cdrom, GError ** error)
{
  if (error) {
    *error = g_error_new (GNOME_CDROM_ERROR,
	GNOME_CDROM_ERROR_NOT_IMPLEMENTED,
...
static gboolean
cdrom_ffwd (GnomeCDRom * cdrom, GError ** error)
{
  if (error) {
    *error = g_error_new (GNOME_CDROM_ERROR,
	GNOME_CDROM_ERROR_NOT_IMPLEMENTED,
...
cdrom_play (GnomeCDRom * cdrom,
    int start_track,
    GnomeCDRomMSF * start,
    int finish_track, GnomeCDRomMSF * finish, GError ** error)
{
  if (error) {
    *error = g_error_new (GNOME_CDROM_ERROR,
	GNOME_CDROM_ERROR_NOT_IMPLEMENTED,
...
static gboolean
cdrom_pause (GnomeCDRom * cdrom, GError ** error)
{
  if (error) {
    *error = g_error_new (GNOME_CDROM_ERROR,
	GNOME_CDROM_ERROR_NOT_IMPLEMENTED,
...
static gboolean
cdrom_stop (GnomeCDRom * cdrom, GError ** error)
{
  if (error) {
    *error = g_error_new (GNOME_CDROM_ERROR,
	GNOME_CDROM_ERROR_NOT_IMPLEMENTED,
...
static gboolean
cdrom_rewind (GnomeCDRom * cdrom, GError ** error)
{
  if (error) {
    *error = g_error_new (GNOME_CDROM_ERROR,
	GNOME_CDROM_ERROR_NOT_IMPLEMENTED,
...
static gboolean
cdrom_back (GnomeCDRom * cdrom, GError ** error)
{
  if (error) {
    *error = g_error_new (GNOME_CDROM_ERROR,
	GNOME_CDROM_ERROR_NOT_IMPLEMENTED,
...
cdrom_get_status (GnomeCDRom * cdrom,
    GnomeCDRomStatus ** status, GError ** error)
{
  if (error) {
    *error = g_error_new (GNOME_CDROM_ERROR,
	GNOME_CDROM_ERROR_NOT_IMPLEMENTED,
...
static gboolean
cdrom_close_tray (GnomeCDRom * cdrom, GError ** error)
{
  if (error) {
    *error = g_error_new (GNOME_CDROM_ERROR,
	GNOME_CDROM_ERROR_NOT_IMPLEMENTED,
...
cdrom_get_cddb_data (GnomeCDRom * cdrom,
    GnomeCDRomCDDBData ** data, GError ** error)
{
  if (error) {
    *error = g_error_new (GNOME_CDROM_ERROR,
	GNOME_CDROM_ERROR_NOT_IMPLEMENTED,
...
static gboolean
cdrom_set_volume (GnomeCDRom * cdrom, int volume, GError ** error)
{
  if (error) {
    *error = g_error_new (GNOME_CDROM_ERROR,
	GNOME_CDROM_ERROR_NOT_IMPLEMENTED,
...
static gboolean
cdrom_is_cdrom_device (GnomeCDRom * cdrom, const char *device, GError ** error)
{
  if (error) {
    *error = g_error_new (GNOME_CDROM_ERROR,
	GNOME_CDROM_ERROR_NOT_IMPLEMENTED,
...
static gboolean
cdrom_set_device (GnomeCDRom * cdrom, const char *device, GError ** error)
{
...
static gboolean
cdrom_open_dev (GnomeCDRom * cdrom, GError ** error)
{
  if (cdrom->priv->open_count++ == 0) {
    cdrom->fd = open (cdrom->priv->device, O_RDONLY | O_NONBLOCK);
...
gboolean
gnome_cdrom_eject (GnomeCDRom * cdrom, GError ** error)
{
...
gboolean
gnome_cdrom_next (GnomeCDRom * cdrom, GError ** error)
{
  GnomeCDRomClass *klass;
...
gboolean
gnome_cdrom_fast_forward (GnomeCDRom * cdrom, GError ** error)
{
  GnomeCDRomClass *klass;
...
gboolean
gnome_cdrom_pause (GnomeCDRom * cdrom, GError ** error)
{
  GnomeCDRomClass *klass;
...
gboolean
gnome_cdrom_stop (GnomeCDRom * cdrom, GError ** error)
{
  GnomeCDRomClass *klass;
...
gboolean
gnome_cdrom_rewind (GnomeCDRom * cdrom, GError ** error)
{
  GnomeCDRomClass *klass;
...
gboolean
gnome_cdrom_back (GnomeCDRom * cdrom, GError ** error)
{
  GnomeCDRomClass *klass;
...
gboolean
gnome_cdrom_close_tray (GnomeCDRom * cdrom, GError ** error)
{
  GnomeCDRomClass *klass;
...
gnome_cdrom_status_changed (GnomeCDRom * cdrom, GnomeCDRomStatus * new_status)
{
  ...
  g_signal_emit (G_OBJECT (cdrom),
      gnome_cdrom_signals[STATUS_CHANGED], 0, status_copy);

  g_free (status_copy);
}

static gboolean
msf_equals (GnomeCDRomMSF * msf1, GnomeCDRomMSF * msf2)
{
  if (msf1 == msf2) {
    return TRUE;
  }
  ...
  if (msf1->minute == msf2->minute &&
      msf1->second == msf2->second && msf1->frame == msf2->frame) {
    return TRUE;
  }
...
gboolean
gnome_cdrom_set_device (GnomeCDRom * cdrom, const char *device, GError ** error)
{
  GnomeCDRomClass *klass;
...
gboolean
gnome_cdrom_set_volume (GnomeCDRom * cdrom, int volume, GError ** error)
{
  GnomeCDRomClass *klass;
...
gboolean
gnome_cdrom_open_dev (GnomeCDRom * cdrom, GError ** error)
{
  GnomeCDRomClass *klass;
...
gnome_cdrom_construct (GnomeCDRom * cdrom,
    const char *device,
    GnomeCDRomUpdate update, GnomeCDRomDeviceLifetime lifetime, GError ** error)
{
  ...
  switch (update) {
    ...
    case GNOME_CDROM_UPDATE_CONTINOUS:
      cdrom->priv->update_id =
	  g_timeout_add (CDROM_POLL_TIMEOUT, timeout_update_cd, cdrom);
      break;

...
static void
kt_stars_update_func(GtkWidget *area)
{
  ...
  pmap = (GdkPixmap *)gtk_object_get_data(GTK_OBJECT(area), "pixmap");
  ...
  if ((w != ww) || (h != hh))
    {
      ...
      gtk_object_set_data(GTK_OBJECT(area), "pixmap", pmap);
    }
...
/*  area = gtk_drawing_area_new();*/
  gtk_object_set_data(GTK_OBJECT(area), "pixmap", pixmap);
  return area;
}
...
void
kt_stars_start(GtkWidget *s, gint width, gint height)
{
  ...
  if (first_time) {
/*
    gtk_widget_set_usize(s, width, height);
*/
    gtk_widget_realize(s);
    first_time = 0;
  }
  timeout_tag = gtk_timeout_add(50, (GtkFunction)kt_stars_update_func, s);
}

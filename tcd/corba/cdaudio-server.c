...
int main(int argc, char *argv[])
{
    ...
    orb = gnome_CORBA_init("GNOME_CDAudio Server", "0.0", &argc, argv, &ev);
    ...
    g_print("registering...\n");
    goad_server_register(name_service, cdaudio_client, "cdaudio", "object", &ev);
    g_print("registered\n");
    retval = CORBA_ORB_object_to_string(orb, cdaudio_client, &ev);
    g_print("%s\n", retval);
    CORBA_free(retval);

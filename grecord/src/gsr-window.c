          ...
          gboolean saveas)
{
	...
	if (saveas && g_file_test (name, G_FILE_TEST_EXISTS)) {
			    ...
			    GSRWindow *window)
{
	...
	case GTK_RESPONSE_OK:
		...
		if (name != NULL) {
			...
			do_save_file (window, name, TRUE /* Is save as */);
			g_free (name);
		}
	   ...
	   const char *path)
{
	if (window->priv->filename == NULL ||
		...
		do_save_file (window, window->priv->filename, FALSE /* Not Save as */);
	}
}
	    ...
	    const char *path)
{
	...
	if (mixer_path == NULL) {
		g_warning (_("%s is not installed in the path."), "gst-mixer");
		return;
	}
